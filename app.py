...
# ─────────────────────────────────────────────────────────
# [공통 함수] Tab 1 랭킹 계산을 위한 데이터 로더
# ─────────────────────────────────────────────────────────
def load_ranking_data():
    """
    Tab 1에서 랭킹을 계산하기 위해 Tab 2와 동일한 '공급량(계획_실적).xlsx' 파일을 읽습니다.
    """
    ...
    try:
        ...
        # '일별실적' 시트 찾기
        sheet_name = next((s for s in xls.sheet_names if "일별" in s), None)
...
# ==============================================================================
# [탭 1] 도시가스 공급실적 관리 (형님 요청대로 수정 완료)
# ==============================================================================
def run_tab1_management():
    ...
    # [수정된 랭킹 함수] Tab 2 데이터 소스를 사용하여 정확한 순위 산출
    def get_historical_ranks(current_val_gj, target_date):
        # 1. Tab 2와 동일한 원본 데이터 로드
        df_hist = load_ranking_data()
        
        if df_hist is None or df_hist.empty:
            return None

        # 2. 중복 방지 (이미 저장된 '오늘' 데이터 제외)
        df_hist = df_hist[df_hist['일자'] != target_date]

        # 3. 전체 랭킹 (과거 데이터 + 현재 입력값)
        all_vals = pd.concat([df_hist['val_gj'], pd.Series([float(current_val_gj)])])
        rank_all = (all_vals > current_val_gj).sum() + 1
        
        # 4. 동월 랭킹
        hist_month = df_hist.loc[df_hist['일자'].dt.month == target_date.month, 'val_gj']
        month_vals = pd.concat([hist_month, pd.Series([float(current_val_gj)])])
    ...
    with col_date:
        # [수정] 디폴트 날짜를 데이터의 최신 날짜로 설정
        valid_data = df[df['실적(GJ)'] > 0]
        default_date = valid_data['날짜'].max() if not valid_data.empty else df['날짜'].max()
...
# ==============================================================================
# [탭 2] 공급량 분석 (형님이 주신 코드 그대로 적용 + 들여쓰기 수정)
# ==============================================================================
def run_tab2_analysis():
    ...
    def supply_daily_main_logic(day_df, month_df, sel_year, sel_month, key_prefix):
        ...
        if not month_all.empty:
            ...
            # [Highlight Card] 선택월의 최고 실적 표시 (Tab 1에서 입력한 값 포함)
            ...
            if not this_df.empty:
                ...
                # Highlight Card 렌더링
                st.markdown(f"""
                <div style="background-color:#e0f2fe;padding:15px;border-radius:10px;border:1px solid #bae6fd;margin-bottom:20px;">
                    ...
                    <div style="font-size:16px; margin-top:5px; color:#333;">
                        공급량: <b>{max_val_gj:,.1f} GJ</b> ➡️ 
                        <span style="background-color:#fff; padding:2px 8px; border-radius:5px; border:1px solid #ddd; margin-left:5px;">🏆 역대 전체 <b>{rank_total}위</b></span>
