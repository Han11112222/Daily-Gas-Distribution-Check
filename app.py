...
# [Han형님 정보] 구글 시트 ID (URL에서 추출)
# 원본 URL: https://docs.google.com/spreadsheets/d/1GLyrA8snj7ffku8ff-3nJ_G4tjBC6SRWBMOInadjgrQ/edit
SHEET_ID = "1GLyrA8snj7ffku8ff-3nJ_G4tjBC6SRWBMOInadjgrQ"

# 로컬(깃허브)에 있는 기본 실적/계획 파일명
...
# [폰트] 한글 폰트 설정 (시스템 폰트 자동 감지)
def set_korean_font():
    # 1. 나눔고딕 파일이 있으면 최우선 적용
    ttf = Path(__file__).parent / "NanumGothic-Regular.ttf"
    if ttf.exists():
        fm.fontManager.addfont(str(ttf))
        mpl.rcParams["font.family"] = "NanumGothic"
    else:
        # 2. 파일이 없으면 시스템 폰트 중에서 한글 폰트 탐색
        ...
        if 'AppleGothic' in font_list:
            ...
            mpl.rcParams["font.family"] = 'sans-serif' # 기본값
...
def load_data_flexible(sheet_type="daily"):
    """
    sheet_type: 'daily' (일별실적) 또는 'monthly' (월별계획)
    """
    df = None
    
    # 1. 구글 스프레드시트 (CSV Export 방식 - 에러 방지)
    if "구글" in data_source_option:
        try:
            # gid=0은 첫번째 시트(일별실적 가정), 월별 시트의 GID를 안다면 gid=12345 식으로 변경 필요
            # 현재는 Han형님의 시트 구조상 첫번째 시트가 '일별실적'이라고 가정합니다.
            gid = "0" 
            csv_url = f"https://docs.google.com/spreadsheets/d/{SHEET_ID}/export?format=csv&gid={gid}"
            
            df = pd.read_csv(csv_url)
            
            # [보정] 구글시트 CSV 로드 시 헤더 정리
...
def run_tab1_management():
    ...
    if df.empty:
        st.warning("⚠️ 데이터를 불러올 수 없습니다. 좌측 사이드바에서 '데이터 원본'을 변경하거나 파일을 확인해주세요.")
        # 빈 껍데기 생성 (에러 방지)
    ...
    if not df_plan.empty:
        # 계획 데이터가 있으면 날짜 기준으로 병합 (Outer join to keep all dates)
    ...
    with col_date:
        # 실적이 있는 마지막 날짜를 기본값으로
        last_act_date = df[df['실적(GJ)'] > 0]['날짜'].max()
        if pd.isna(last_act_date): last_act_date = datetime.now().date()
    ...
    # m3 단위 천단위 절삭 (10,000 이상이면 천단위로 가정하고 나누기, 아니면 그대로) -> Han형님 데이터에 맞춰 조정
    def to_thou(val):
        return val / 1000.0
...
def run_tab2_analysis():
    ...
    # 데이터 로드 (분석탭은 일별 데이터를 기반으로 집계하여 보여주는 것이 가장 정확함)
    raw_df = load_data_flexible("daily")
    df = process_daily_data(raw_df)
    ...
    fig = px.line(monthly_agg, x='월', y='실적(GJ)', color='연', markers=True,
                  title="연도별 월간 공급량 추이", symbol='연')
    ...
    scatter_df = scatter_df[scatter_df['실적(GJ)'] > 10] # 0인 값 제외
